...
static void r2flutter_subcmd_help(RCore *core, char action) {
	...
	case 'i':
		r_cons_printf (core->cons,
			"Usage: r2flutter -i[jr*]  dump instruction table entries\n"
			"| -i        plain text output\n"
			"| -ie       dump Dart code entrypoint; with -* mark instruction snapshots as dword arrays\n"
			"| -ij       JSON output\n"
			"| -ir, -i*  r2 commands output\n");
		break;
	case 'e':
		r_cons_printf (core->cons,
			"Usage: r2flutter -ie[jr*]  dump Dart code entrypoint\n"
			"| -ie       plain text output\n"
			"| -iej      JSON output\n"
			"| -ier, -ie*  r2 commands output; mark instruction snapshots as dword arrays\n");
		break;
	default:
		r2flutter_help (core);
		break;
...
static void r2flutter_help(RCore *core) {
	r_cons_printf (core->cons,
		...
		"| r2flutter -E[jr*]  dump Dart code entrypoint\n"
		"| r2flutter -f[jr*]  dump recovered functions\n"
		"| r2flutter -H[HH]   dump Dart AOT snapshot header info\n"
		"| r2flutter -i[jr*]  dump instruction table entries\n"
		"| r2flutter -ie[jr*] dump Dart code entrypoint; with -r/-* mark instruction snapshots as dword arrays\n"
		...
		"| r2flutter -z[jr*]  dump reliable ObjectPool-referenced strings\n"
		...
		"Modifiers: q=quiet, j=json, r/*=r2 commands\n");
}
...
static bool r2flutter_run_cmd(RCore *core, DartCtx *dctx, const R2FlutterCmd *cmd) {
	...
	case 'R':
		...
		return true;
	case 'S':
		out = dart_pool_dump_sbom (dctx, core->io && core->io->desc? core->io->desc->name: NULL, cmd->fmt);
		break;
	case 'z':
